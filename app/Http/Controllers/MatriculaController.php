...
class MatriculaController extends Controller
{
        ...
        if (empty($colegiado)) {
            ...
            if (!empty($colegiado)) {
                ...
                /* $cursos = Colegiado::join('matriculas', 'matriculas.colegiado_id', '=', 'colegiados.id')
                    ...
                    ->get(); */
                $cursos = Persona::join('matriculas', 'matriculas.persona_id', '=', 'personas.id')
                    ->join('cursos', 'cursos.id', '=', 'matriculas.curso_id')
                    ->select('cursos.id', 'cursos.nombre', 'cursos.temario', 'cursos.estado')
                    ->where('matriculas.persona_id', '=', $colegiado->id)
                    ->get();
            }
        } else {
    ...
    public function certificadodatos($idcurso, $idcolegiado){
        //Recuperar todos los productos de la db
        $colegiado=Colegiado::find($idcolegiado);
        if(empty($colegiado)){
            $colegiado=Persona::find($idcolegiado);
        }
        $cursos=Curso::find($idcurso);
        return view('certificados.usuarios.datos', compact('colegiado', 'cursos'));
    }
